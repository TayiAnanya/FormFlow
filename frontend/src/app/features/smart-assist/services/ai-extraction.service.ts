...
export class AIExtractionService {
  ...
  /**
   * Free-form structured JSON generation (Smart Banking Advisor, etc.).
   * Masks sensitive identifiers before calling the LLM; does not restore onto a form schema.
   */
  async generateStructuredJson(prompt: string): Promise<AIExtractionResult> {
    ...
    try {
      const result = await this.provider.generateStructuredJson(privacy.maskedText);
      if (!result.success || !result.data) {
        return {
          success: false,
          errorMessage: result.errorMessage ?? AI_EXTRACTION_FAILURE_MESSAGE,
      ...
      return {
        success: true,
        data: sanitizeAIExtractionData(result.data),
        rawResponse: result.rawResponse,
      };
    } catch {
      return {
