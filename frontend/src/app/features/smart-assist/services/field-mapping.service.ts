...
/**
 * Resolves an extracted property onto a schema field using the active schema only.
...
export class FieldMappingService {
  /**
   * Maps label-extracted values onto the active schema.
  ...
  mapToSchemaFields(schema: FormSchema, extracted: ExtractedDocumentData): MappedFormValues {
    ...
    for (const [property, value] of Object.entries(extracted)) {
      ...
      const mappedLogTarget =
        field.type === 'dropdown' || field.type === 'multiselect'
          ? formatMappedLogValue(mappedValue)
          : field.key;
...
function formatMappedLogValue(value: unknown): string {
