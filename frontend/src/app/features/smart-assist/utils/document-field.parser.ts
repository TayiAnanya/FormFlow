...
export function findValueAfterLabel(
  ...
  const flattenedPattern = otherLabels
    ? `${escapeRegExp(label)}\\s*[:\\-–—]?\\s+(.+?)(?=(?:\\s+(?:${otherLabels})\\b)|$)`
    : `${escapeRegExp(label)}\\s*[:\\-–—]?\\s+(.+)$`;

  const flattenedMatch = new RegExp(flattenedPattern, 'i').exec(flattened);
 ...
 *
 * Extracted object keys are the schema field keys (e.g. fullName) so
 * FieldMappingService can apply dropdown / multiselect coercions next.
 */
